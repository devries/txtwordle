...
echo $ac_n "Where would you like to install txtwordle? [$defaultprefix]: $ac_c"
read prefix
if [ "$prefix" = "" ]; then
...
if [ $machine != "unknown" ]; then
  ...
  # Copy the binary
  cp ${machine}/txtwordle ${binpath}/txtwordle

  # make binary executable
  chmod -R ugo+x ${binpath}/txtwordle

  # Write some documentation
  echo "txtwordle is now installed in ${binpath}"
else
  arch="$(uname -m)"
